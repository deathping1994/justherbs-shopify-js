setTimeout(function () {
    ...
    $(document).on(`load change`, '.tdf_input_discount', function () {
        console.log("started")
        var token = ""
        $.getJSON('/cart.js', function (cart) { token = cart.token }); $(".tdf_normal_btn").on("click touchstart", function (event) {
            ...
            }).fail(() => { $(".tdf_input_discount")[0].value = basecode; $(".tdf_normal_btn").trigger("custom-event"); });
        });
    });
}, 5000);
